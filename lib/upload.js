...
Upload.prototype.save = function () {
    ...
    if (self.meta.validation && self.meta.validation.summary && !_.isArray(self.meta.validation.summary)) {
...
Upload.prototype.validateResource = function (resource, reportType) {
    var self = this;
    var deferred = Q.defer();
    var action = reportType === 'summary' ? 'validate:summary' : 'validate';
    var args = [
        '-F,',
        '-v','action=' + action,
    ...
    self.logger.log(action + ': ' + resource.path);
...
Upload.prototype.validateResources = function (reportType) {
    var self = this;
    self.logger.log('validating upload: ' + self.meta.id);
    var header = 'file_name,field_name,rule,message,violation_severity,violation_count\n';

    var validates = self.datapackage.resources.map(function (r) {
        return self.validateResource(r, reportType);
    });

    return Q.allSettled(validates).then(function (results) {
        self.logger.log('all violations complete');
        var violations, count, deferred, params, archive;

        if(reportType === 'summary') {
            violations = results.reduce(function (memo, result) {
                if (result.state === 'fulfilled' && result.value.violations && result.value.violations !== '\n') {
                    memo += result.value.violations;
                }
                return memo;
            }, '');

            count = violations.split('\n').reduce(function (c, r) {
                var fields = r.split(',');
                if (fields.length === 6) {
                    c += parseInt(fields[5]);
                }
            ...
            params = {
                Key: self.root + '/validate/violations.zip',
            ...
            self.s3.upload(params).send(function(err, data) {
                ...
                self.logger.log('uploaded zip to: ' + data.Location);
                return deferred.resolve();
            });

            archive = _.chain(results)
                .filter(function (r) {
                    return r.state === 'fulfilled' && r.value.violations;
                })
                .pluck('value')
                .reduce(function (archive, resourceViolation) {
                    archive.append(resourceViolation.violations, {name: resourceViolation.resource.path});
                    return archive;
                }, archive)
                .value();
        ...
        .pipe(es.through(function (line) {
            ...
            if (!pgStream) {
                ...
                pgStream.on('error', function (err) {
                    self.logger.error('error importing ' + resource.path);
